...
namespace AccesoDatos
{
    public partial class frmEliminar : Form
    {
        ...
        SqlConnection conexion = new SqlConnection(@"server=LAPTOP-3N4L4LQP\SQLEXPRESS; database=TIC; Integrated Security=true");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("¿ESTAS SEGURO QUE DESEAS ACTUALIZAR?", "CONFIRMAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    conexion.Open();
                    string actualizar = "UPDATE personas SET apellidos=@apellido, nombres=@nombre, fechaNacimiento=@fechadenacimiento, peso=@peso WHERE cedula=@cedula";
